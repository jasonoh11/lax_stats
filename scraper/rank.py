...
def power_iteration(markov_matrix, tolerance=1e-8, max_iter=200):
    ...
    # Initialize with equal ranking power
    ranking = np.ones(n) / n

    for iteration in range(max_iter):
        new_ranking = markov_matrix.T @ ranking  # Propagate ranking power along the edges
        # Check convergence using L1 norm
        if np.linalg.norm(new_ranking - ranking, 1) < tolerance:
            print(f"Converged after {iteration+1} iterations.")
            return new_ranking / new_ranking.sum()  # Normalize
        ranking = new_ranking

    print("Max iterations reached without full convergence.")
    return ranking / ranking.sum()
...
def calculate_rank(games, ids):
	...
	ranking_vector = power_iteration(markov_matrix)
	scaled_ratings = scale_ratings(ranking_vector)
